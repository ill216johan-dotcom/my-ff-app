...
export function PackagingList({ packagings, selectedId, onSelect, onCreate }: PackagingListProps) {
  ...
  const stats = {
    ...
    totalResponses: packagings.reduce((acc, p) => acc + p.responsesCount, 0),
    unreadMessages: packagings.reduce((acc, p) => acc + p.unreadMessages, 0),
  }
      ...
      <div className="flex-1 p-6 overflow-auto border-r border-border">
        ...
        <div className="flex items-center gap-2 mb-5">
          <div className="relative flex-1">
            ...
            <Input placeholder="Поиск..." className="pl-9 h-9 bg-secondary border-border text-sm" />
          </div>
    ...
    >
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span className="flex items-center gap-1">
          ...
          {packaging.articlesCount} арт.
        </span>
        <span className="flex items-center gap-1">
          <Clock className="w-3 h-3" />
          {new Date(packaging.deadline).toLocaleDateString("ru-RU", { day: "numeric", month: "short" })}
        </span>
        <span className="font-medium text-foreground">{packaging.budget}</span>
      </div>
    </button>
  )
}
