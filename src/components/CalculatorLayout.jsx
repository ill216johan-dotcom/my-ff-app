    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${isDarkMode ? 'bg-[#0a0a0a]' : 'bg-slate-50'}`}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white dark:bg-black border-b border-slate-200 dark:border-neutral-800 shadow-sm">
        <div className="w-full px-6 lg:px-8">
          <div className="max-w-7xl mx-auto flex items-center justify-center h-16 flex-nowrap gap-4 lg:gap-8">
            {/* Logo */}
            ...
            <button
              onClick={toggleTheme}
              className="p-2 rounded-md text-slate-500 dark:text-neutral-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-neutral-800 transition-colors flex-shrink-0"
            ...
            {authLoading ? (
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-neutral-900 rounded-md border border-slate-200 dark:border-neutral-800 max-w-xs">
                  ...
                  <span className="whitespace-nowrap text-sm font-medium text-slate-900 dark:text-white truncate sm:max-w-[120px] md:max-w-none">
                ...
                <button
                  ...
                  className="p-2 rounded-md text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors flex-shrink-0"
                  title="Выйти"
                >
                  <LogOut size={18} />
                </button>
              ...
              <Link
                to="/auth"
                className="flex items-center p-2 md:px-4 md:py-2 md:gap-1.5 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-md transition-colors shadow-sm whitespace-nowrap flex-shrink-0"
              >
                <LogIn size={16} />
          ...
          >
            {navItems.map((item) => (
              <Link
                key={item.path}
                to={item.path}
                className={`whitespace-nowrap flex-shrink-0 px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${
                  isActive(item)
                    ? 'text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20'
                    : 'text-slate-600 dark:text-neutral-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-neutral-800'
                }`}
